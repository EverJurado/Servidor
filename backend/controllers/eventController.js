...
const getEventosConStats = async (req, res) => {
  try {
    const query = `
      SELECT e.*, 
        COUNT(a.id_agenda) AS total_inscritos,
        COUNT(CASE WHEN a.asistio = true THEN 1 END) AS total_asistentes
      FROM eventos e
      LEFT JOIN agenda a ON e.id_evento = a.id_evento
    ...
    console.error("Error al obtener eventos con stats:", error);
    res.status(500).json({ error: "Error al obtener eventos con estadísticas" });
  }
};


module.exports = { crearEvento, getEventos, getTodos, getEventosConStats };
